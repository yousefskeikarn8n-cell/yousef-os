...
export default function Systems({ systems, onToggle, onAdd, onDelete }) {
  ...
  return (
    <div style={{ padding:"24px 16px 80px" }}>
      ...
      <div style={{
        ...
        {[
          { label:"إجمالي",  value:systems.length, sub:"systems" },
          { label:"Live 🟢", value:live,           sub:`/ ${systems.length}` },
      ...
      <div style={{
        ...
        background:G.card, border:`1px solid ${G.border}`, borderRadius:8, overflow:"hidden",
      }}>
        <div style={{ width:8, flexShrink:0 }} />
        {STAGE_LABELS.map((l,i) => (
          <div key={i} style={{
            flex:1, textAlign:"center", padding:"7px 0",
            fontSize:9, color:G.grey, fontWeight:700, letterSpacing:"0.06em",
            borderRight: i<5 ? `1px solid ${G.border}` : "none",
          }}>{l}</div>
        ))}
      ...
      {systems.map(s => {
        ...
        return (
          <div key={s.id} style={{
            marginBottom:10, padding:"13px 12px",
            ...
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:10, gap:8 }}>
              <div style={{ flex:1 }}>
                <div style={{ fontSize:11, fontWeight:600, lineHeight:1.4, color:G.white }}>{s.name}</div>
            ...
            <div style={{ display:"flex", gap:5, alignItems:"center" }}>
              {s.stages.map((done,i) => (
                <button key={i} onClick={()=>onToggle(s.id,i)} title={STAGE_LABELS[i]} style={{
                  ...
                  color: done ? G.green : G.grey,
                  ...F, fontWeight:700, fontSize:9, cursor:"pointer",
                  display:"flex", alignItems:"center", justifyContent:"center",
                  boxShadow: done ? `0 0 7px ${G.greenBorder}` : "none",
                  transition:"all 0.2s",
                }}>{done ? "✓" : i+1}</button>
              ))}
